...
CREATE TABLE IF NOT EXISTS public.loyalty_history (
  ...
  reason TEXT NOT NULL, -- e.g., 'Purchase #ORD-123', 'Referral Bonus'
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 2. Categories & Collections (Normalized)
CREATE TABLE IF NOT EXISTS public.categories (
  id TEXT PRIMARY KEY, -- e.g., 'tops', 'bottoms'
  name TEXT NOT NULL,
  description TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.collections (
  id TEXT PRIMARY KEY, -- e.g., 'summer-2025'
  name TEXT NOT NULL,
  description TEXT,
  banner_url TEXT,
...
CREATE TABLE IF NOT EXISTS public.products (
  ...
  is_active BOOLEAN DEFAULT TRUE,
  -- Dynamic Data
  color_variants JSONB DEFAULT '[]', -- Array of { color: string, hex: string, image: string }
  additional_images JSONB DEFAULT '[]', -- Array of strings
  accordion_items JSONB DEFAULT '[]', -- Array of { title: string, content: string }
  key_highlights JSONB DEFAULT '[]', -- Array of strings
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.kits (
  ...
  content JSONB DEFAULT '{}', -- Large JSON for complex page content (hero, about, benefits, usage, faq)
  is_active BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.inventory_movements (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  product_id TEXT REFERENCES public.products(id) ON DELETE CASCADE,
  movement_type TEXT NOT NULL CHECK (movement_type IN ('sale', 'restock', 'adjustment', 'return', 'reservation', 'reservation_release')),
  quantity_change INTEGER NOT NULL,
  quantity_before INTEGER NOT NULL,
  quantity_after INTEGER NOT NULL,
  reference_id TEXT, -- Order ID or reference
  reason TEXT,
...
CREATE TABLE IF NOT EXISTS public.promo_codes (
  id TEXT PRIMARY KEY, -- e.g., 'BENANG10'
  description TEXT,
...
CREATE TABLE IF NOT EXISTS public.order_items (
  ...
  kit_id TEXT REFERENCES public.kits(id) ON DELETE SET NULL, -- Support for purchasing kits
...
CREATE TABLE IF NOT EXISTS public.site_settings (
  ...
  brand_stats JSONB DEFAULT '{}', -- { customers, reviews, itemsSold }
...
CREATE TABLE IF NOT EXISTS public.flash_sale_products (
  ...
  product_id TEXT REFERENCES public.products(id) ON DELETE CASCADE,
  sale_price DECIMAL(12, 2), -- Optional override for product price
  created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TRIGGER update_kits_ts BEFORE UPDATE ON kits FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
CREATE TRIGGER update_inventory_ts BEFORE UPDATE ON inventory FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
CREATE TRIGGER update_orders_ts BEFORE UPDATE ON orders FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
CREATE TRIGGER update_site_settings_ts BEFORE UPDATE ON site_settings FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
CREATE TRIGGER update_cms_pages_ts BEFORE UPDATE ON cms_pages FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
CREATE TRIGGER update_flash_sales_ts BEFORE UPDATE ON flash_sales FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
...
CREATE TABLE IF NOT EXISTS public.shipments (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  order_id TEXT REFERENCES public.orders(id) ON DELETE CASCADE,
  logistics_provider TEXT, -- e.g., 'biteship', 'rajaongkir', 'shipper'
  logistics_order_id TEXT,
  courier_company TEXT NOT NULL, -- e.g., 'JNE', 'SiCepat'
  courier_type TEXT NOT NULL, -- e.g., 'REG', 'YES'
...
CREATE TRIGGER update_payment_transactions_ts BEFORE UPDATE ON payment_transactions FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
CREATE TRIGGER update_user_addresses_ts BEFORE UPDATE ON user_addresses FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
CREATE TRIGGER update_shipments_ts BEFORE UPDATE ON shipments FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
...
CREATE TRIGGER update_email_sequences_ts BEFORE UPDATE ON email_sequences FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
CREATE TRIGGER update_checkout_abandons_ts BEFORE UPDATE ON checkout_abandons FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
