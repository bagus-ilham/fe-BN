...
export default function ResendConfirmationEmail({
  ...
  // Reset loading saat komponen dipasang kembali atau saat pengguna kembali
  useEffect(() => {
    // Reset saat pemasangan (jika pengguna kembali)
    setLoading(false);

    // Reset saat pengguna menggunakan tombol kembali di browser
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const errorMsg = result.error || "Gagal mengirim ulang email";
        setError(errorMsg);
        onError?.(errorMsg);
      }
    } catch (err: unknown) {
      const errorMsg = err instanceof Error ? err.message : "Gagal mengirim ulang email";
      setError(errorMsg);
      onError?.(errorMsg);
    } finally {
      setLoading(false);
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="font-medium mb-1">Email konfirmasi berhasil dikirim ulang!</p>
            <p className="text-xs opacity-80">
              Silakan periksa kotak masuk dan folder spam Anda. Email mungkin memerlukan beberapa
              menit untuk sampai.
            </p>
          </div>
        </div>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="text-sm text-yellow-800 mb-2 font-medium">
            Konfirmasi email Anda untuk masuk
          </p>
          <p className="text-xs text-yellow-700 mb-3">
            Belum menerima email? Periksa kotak masuk dan folder spam Anda, atau
