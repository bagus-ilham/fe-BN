...
function StarInput({
  ...
  return (
    <div className="flex gap-1" role="group" aria-label="Penilaian">
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          key={star}
          type="button"
...
export default function ReviewForm({ productId, onSuccess }: ReviewFormProps) {
  ...
  const handleFileChange = useCallback((file: File) => {
    ...
    if (!ALLOWED_MIME.includes(file.type)) {
      setImageError("Format tidak valid. Gunakan JPG, PNG, atau WEBP.");
      return;
    }
    if (file.size > MAX_SIZE) {
      setImageError("Gambar terlalu besar. Maksimal 3 MB.");
      return;
    }
    setPendingFile(file);
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (rating < 1) {
        setMessage({
          type: "error",
          text: "Pilih penilaian (1 sampai 5 bintang).",
        });
        return;
      }
      ...
      if (pendingFile) {
        ...
        try {
          ...
          if (!res.ok) {
            ...
            setMessage({
              type: "error",
              text: data.error ?? "Gagal mengunggah gambar. Coba lagi.",
            });
            return;
          }
          ...
          setMessage({
            type: "error",
            text: "Gagal mengunggah gambar. Periksa koneksi Anda.",
          });
          return;
        }
      }
      ...
      if (result.success) {
        setMessage({ type: "success", text: result.message || "Avaliação enviada!" });
        setRating(0);
        setText("");
        setAuthorName("");
        setAuthorEmail("");
        ...
        setMessage({ type: "error", text: result.error || "Erro ao enviar avaliação." });
      }
    },
    [
      productId,
  ...
  return (
    ...
    >
      ...
      <p className="text-[10px] text-brand-softblack/50 mt-4">
        Ulasan Anda akan diterbitkan setelah proses moderasi.
      </p>
    </motion.form>
  );
}
