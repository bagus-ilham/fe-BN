...
interface ProductReviewsProps {
  /** ID produk (prod_1, prod_2...) */
  productId?: string;
  /** ID kit — jika diisi, menampilkan testimoni kit atau produk dalam kit */
  kitId?: string;
  /** ID produk dalam kit (untuk mencari testimoni produk yang disertakan) */
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`Foto ulasan oleh ${author}`}
      >
        <motion.div
        ...
        >
          ...
          <p className="text-center text-[10px] uppercase tracking-[0.25em] text-white/60 mt-3">
            {author}
          </p>
        </motion.div>
      </motion.div>
      ...
      >
        {review.imageUrl && (
          <button
            ...
            aria-label={`Lihat foto oleh ${review.author}`}
          >
            <Image
              src={review.imageUrl}
              alt={`Foto ulasan oleh ${review.author}`}
              fill
  ...
  return (
    ...
    >
      ...
      <p className="text-xs font-light text-brand-softblack/65 leading-relaxed max-w-sm">
        Setelah pembelian, bagikan pengalaman Anda dan bantu pelanggan lain menemukan manfaat dari benangbaju.
      </p>
    </motion.div>
  );
}
